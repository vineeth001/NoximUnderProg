...
NoximFlit NoximProcessingElement::nextFlit()
{
    ...
    flit.hop_no = 0;
    flit.phase = packet.phase;				//vinu: added for VALIANT    
...
/*	for (uint i = 0; i < NoximGlobalParams::hotspots.size(); i++) {
    ...
    p.phase = false;			//vinu: phase info, similarly in every other routing algo
...
//TODO: Need to make sure that during packet formation, dst_id !== inter_dst_id
	...
	if(NoximGlobalParams::routing_algorithm == ROUTING_VALIANT) 
	{
		...
		p.phase = true;					//vinu: phase info, similarly in every other routing algo
	}
        else
	{
		p.inter_dst_id = 0;
