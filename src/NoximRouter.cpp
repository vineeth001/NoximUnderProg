...
void NoximRouter::rxProcess()
{
	...
	for (int i = 0; i < DIRECTIONS + 1; i++) {
		...
		if (NoximGlobalParams::verbose_mode > VERBOSE_OFF) {
              		...
              		stringstream ss;	//to read receive activity to file for each router
			ss << local_id;
			string file_name= ss.str();
			string b = "_rx"; 
			file_name.append(b);
			//strcat (file_name,"_rx");
			ofstream trace_file;
...
void NoximRouter::txProcess()
{
	...
	for (int j = 0; j < DIRECTIONS + 1; j++) {
		...
		if (flit.flit_type == FLIT_TYPE_HEAD) {
		    ...
		    route_data.inter_dst_id = flit.inter_dst_id;    //vinu: for i_d_i
		    route_data.dst_id = flit.dst_id;
		    route_data.phase = flit.phase;			//vinu: phase info:
		    route_data.dir_in = i;

		    int o = route(route_data);			//vinu: this is where we start route calculation

		    if (reservation_table.isAvailable(o)) {
			reservation_table.reserve(i, o);

			//////////////////////////vinu:  phase change for valiant ///////////////////////////////
		        if (NoximGlobalParams::routing_algorithm == ROUTING_VALIANT)
		        {
				...
				flit.phase = false;
				buffer[i].Pop(); //pop it from the deque
			        buffer[i].PushFront(flit);
    ...
    int >NoximRouter::routingFunction(const NoximRouteData & route_data)
{
    ...
    NoximCoord dst_coord;
    if(NoximGlobalParams::routing_algorithm == ROUTING_VALIANT && route_data.phase == true)   //vinu: check if it contradicts for phase
    {
	dst_coord = id2Coord(route_data.inter_dst_id);
	//cout<<"Error is from inter_dst_id"<<endl;
    }
    else
    {
	dst_coord = id2Coord(route_data.dst_id);
    }

    int dir_in = route_data.dir_in;

    switch (NoximGlobalParams::routing_algorithm) {
    case ROUTING_VALIANT:					//vinu: valiant algo
	return routingValiant(position, dst_coord);
...
{
    stats.power.Routing();		//accounting for routing power

    if (route_data.dst_id == local_id && route_data.phase == false)    //met the destination
	return DIRECTION_LOCAL;
//TODO: Need to make sure that during packet formation, dst_id !== inter_dst_id
...
vector < int >NoximRouter::routingValiant(const NoximCoord & current,                        //vinu: for valiant
