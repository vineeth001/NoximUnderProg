...
struct NoximPacket {
    ...
    bool phase;			// vinu: for valiant algo, phase flag

    // Constructors
    NoximPacket() { }
    NoximPacket(const int s, const int d, const int inter_d, const double ts, const int sz, bool ph) {		//vinu: added inter_d and phase flag
	make(s, d, inter_d, ts, sz, ph);
    }

    void make(const int s, const int d, const int inter_d, const double ts, const int sz, bool ph) {
	src_id = s;
	dst_id = d;
...
struct NoximRouteData {
    int current_id;
    int src_id;
    int inter_dst_id;		//vinu: for valiant algo
    int dst_id;			
    int dir_in;			// direction from which the packet comes from
    bool phase;			//vinu: for phase in valiant's algo
};
...
inline ostream & operator <<(ostream & os, const NoximFlit & flit)
{
	...
	os << "Destination Tile[" << flit.dst_id << "]" << endl;
        if(NoximGlobalParams::routing_algorithm == ROUTING_VALIANT) 		//vinu: for valiant's algo
	{
		os << "Intermediate Router[" << flit.inter_dst_id << "]" << endl;
		os << "Current Phase["<<flit.phase<<"]" << endl;
	}
...
//TODO : need to recalculate hopcount etc
	...
	os << ", seq: " << flit.sequence_no << ", " << flit.src_id << "-->" << flit.inter_dst_id << "-->" << flit.dst_id <<", phase:" << flit.phase << "]";
	else{
		os << "Error\n";
